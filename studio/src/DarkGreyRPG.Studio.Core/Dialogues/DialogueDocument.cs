...
public sealed class DialogueDocument : INotifyPropertyChanged
{
    ...
    private DialogueDocument(DialogueResource resource, string? sourcePath, bool isNew) { _resource = resource; _sourcePath = sourcePath; _isNew = isNew; _saved = isNew ? null : resource.WithId(resource.Id); Revalidate(); }
    ...
    public string? SourcePath => _sourcePath; public bool IsNew => _isNew; public bool IsDirty => _isNew || !SavedContentEquals(); public IReadOnlyList<ValidationIssue> ValidationIssues => _issues; public IReadOnlyList<ValidationIssue> ValidationErrors => _issues.Where(i => i.Severity == ValidationSeverity.Error).ToArray();
    ...
    internal void MarkSaved(string path) { _sourcePath = Path.GetFullPath(path); _isNew = false; _resource = ToResource(); _saved = _resource.WithId(_resource.Id); Revalidate(); PropertyChanged?.Invoke(this, new(nameof(IsNew))); PropertyChanged?.Invoke(this, new(nameof(IsDirty))); }
