...
public static class GraphNodeDefinitionRegistry
{
    ...
    private static List<GraphNodeDefinition> BuildDefinitions()
    {
        ...
        return
        [
            Node("start", GraphScope.StoryFlow, "开始", "流程", required: true, unique: true, kinds: flowOnly,
                properties: [
            ...
            Node("session", GraphScope.StoryFlow, "会话", "聚合", kinds: all,
                ports: [In("flow_in", "Flow In", GraphInterfaceKind.Flow, 0), In("logic_in", "Logic In", GraphInterfaceKind.Logic, 1)],
            ...
            Node("action", GraphScope.StoryFlow, "动作", "动作", kinds: flowOnly,
                ...
                [
                    ...
                    new GraphPropertyDefinition(CanonicalStoryActionSchema.MetadataProperty, JsonValueKind.Number),
            ...
            Node("enter_story", GraphScope.StoryFlow, "进入剧情", "剧情", kinds: flowOnly,
                ports: [In("flow_in", "Flow In", GraphInterfaceKind.Flow, 0)],
            ...
            Node("activate", GraphScope.Task, "激活", "任务", required: true, unique: true, kinds: logicOnly,
                ports: [Out("logic_out", "Logic Out", GraphInterfaceKind.Logic, 0)]),
            Node("objective", GraphScope.Task, "目标", "任务", kinds: logicOnly,
                ports: [In("logic_enable", "Logic Enable", GraphInterfaceKind.Logic, 0), Out("logic_status", "Logic Status", GraphInterfaceKind.Logic, 1)],
                properties: [
