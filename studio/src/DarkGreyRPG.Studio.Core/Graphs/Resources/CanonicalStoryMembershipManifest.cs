...
public static class CanonicalStoryMembershipSerializer
{
    ...
    private static readonly string[] MembershipMembers = ["actors", "sessions", "tasks"];
    ...
    public static string Serialize(CanonicalStoryMembershipManifest manifest, bool indented = true)
    {
        ...
        }))
        {
            ...
            writer.WriteNumber("schema_version", CanonicalStoryMembershipManifest.CurrentSchemaVersion);
            writer.WriteString("story_id", manifest.StoryId);
            WriteSet(writer, "owned_resources", manifest.SnapshotOwned());
            WriteSet(writer, "referenced_resources", manifest.SnapshotReferenced());
    ...
    public static CanonicalStoryMembershipManifest Deserialize(string json)
    {
        ...
        try
        {
            ...
            if (version != CanonicalStoryMembershipManifest.CurrentSchemaVersion)
            ...
            var manifest = new CanonicalStoryMembershipManifest(
                ...
                ReadSet(Required(root, "owned_resources"), "owned_resources"),
                ReadSet(Required(root, "referenced_resources"), "referenced_resources"))
            {
    ...
    private static void WriteSet(Utf8JsonWriter writer, string name, CanonicalStoryMembershipSet set)
    {
    ...
    private static CanonicalStoryMembershipSet ReadSet(JsonElement element, string path)
    {
        if (element.ValueKind != JsonValueKind.Object)
            throw Failure("story.membership.set.invalid", $"'{path}' must be an object.");
        EnsureExactMembers(element, MembershipMembers, "story.membership.set");
    ...
    private static void EnsureExactMembers(JsonElement element, IEnumerable<string> expected, string codePrefix)
    {
